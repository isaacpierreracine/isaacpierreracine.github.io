...
Pour documenter le projet j'ai créé une page web statique utilisant le modèle Stack de la plateforme HUGO. Le site est hébergé sur Github et a été créé en collaboration avec l'interface web de Claude.ai de Anthropic. J'ai choisi de travailler avec la technologie de page statique parce qu'elle permet de créer une page légère et rapide. Cette approche permet aussi d'intégrer le langage Markdown et évite d'avoir recours au langage HTML. Plus particulièrement j'ai opté pour le modèle Stack pour la simplicité de son apparence simple et versatile.
La documentation qui suit représente le contenu du fichier readme.md de la page web et contient toutes les informations utiles pour comprendre la façon dont la page fonctionne ainsi que toutes les interventions effectuées dans le processus d'adaptation de la page.
Ce document est actualisé constamment et reflète la page dans son état actuel.
